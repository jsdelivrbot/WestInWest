$(document).ready(function(){
    ...
    $('#add-input').on('change', function(){
        ...
        if(addInput.val() != ''){
            ...
            $('#completed').html('File uploaded Succesfully')
    ...
    //     valid = false;
    //     $('#error').html('<div class="alert alert-danger">You cannot add an empty field</div>');
    //     // $('#error').html('<div class"alert alert-danger">You Can not add empty field</div>');
    //   } else {
    //     upload = $('#add_input').val();
    //     $('#error').html('');
    //   }
    //
    //   if(valid === true){
...
function ShowImage(input){
  if(input.files && input.files[0]){
    ...
    reader.onload = function(e){
      $('#show_img').attr('src', e.target.reault);
    }
